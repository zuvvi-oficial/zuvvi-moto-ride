...
### Microetapa 5.10 — Observabilidade mínima de erros do cliente — ✅ IMPLEMENTADA
- **Achado:** o servidor já tinha uma captura de erro bastante boa (`src/lib/error-capture.ts` — expande stack/cause chain completo antes de qualquer `console.error`, inclusive recuperando erros que o h3 engolia em respostas 500 genéricas). O que faltava era o lado do **navegador**: `reportLovableError` (`src/lib/lovable-error-reporting.ts`) só entrega em `window.__lovableEvents`/`__lovableReportRuntimeError`, hooks que **só existem dentro do preview do editor Lovable** — em produção, para um usuário real, isso é no-op silencioso. Ou seja: uma tela branca/erro de JS no celular de um usuário real, hoje, não deixa nenhum rastro em lugar nenhum.
- **Correção (estritamente aditiva, nada removido/alterado do que já existia):**
  - `src/lib/client-error-report.functions.ts` (novo): `registrarErroCliente`, server function **sem** `requireSupabaseAuth` (erro pode acontecer antes do login), que só reconstrói um `Error` e chama `console.error` — reaproveita o mesmo pipeline de expansão já existente em `error-capture.ts`. Nenhuma tabela nova, nenhum dado pessoal persistido.
  - `src/lib/client-error-reporter.ts` (novo): `installGlobalErrorReporting()` (ouve `window.onerror`/`unhandledrejection` globalmente) e `reportClientError()` (usado também no error boundary do TanStack Router). Limite de 5 envios por sessão de página, só de proteção básica contra um componente quebrado entrar em loop — não é rate limiting de servidor (este projeto não tem Redis/KV disponível; documentado como limitação conhecida).
  - `src/routes/__root.tsx`: `ErrorComponent` chama `reportClientError` **junto com** o `reportLovableError` já existente (não substitui); `RootComponent` ganhou um novo `useEffect` isolado (`installGlobalErrorReporting()`) — os `useEffect`s e o JSX que já existiam continuam exatamente iguais.
- **Validação:** `npx tsc --noEmit` e `npx eslint` sem erros novos (comparado arquivo a arquivo contra o estado anterior — inclusive `__root.tsx`, cujos 2 erros pré-existentes de `any` só mudaram de número de linha).
- **NÃO é uma solução completa de observabilidade** — continua sem agregação/alerta automático (ex.: Sentry) e sem dashboard; o que existe agora é: um erro real do navegador de um usuário passa a aparecer no mesmo log estruturado que o servidor já usa, em vez de desaparecer sem deixar rastro. Decisão de contratar uma ferramenta de APM/erro dedicada (novo custo, nova dependência) fica para o responsável do projeto.

### Microetapa 5.11 — Política de Privacidade e Termos de Uso — ✅ IMPLEMENTADA
- **Achado:** a landing page (`src/routes/index.tsx`) já tinha, no rodapé, os links "Privacidade", "Termos" e "Contato" — mas todos apontavam para `href="#"` (âncora morta). O app coleta CPF (cadastro), CNH e documentos de veículo (onboarding do motorista) e localização em tempo real durante a corrida, então operar um piloto real sem uma política de privacidade publicada é uma lacuna de conformidade com a LGPD (Lei nº 13.709/2018), não só um link quebrado.
- **Correção (estritamente aditiva, nenhuma rota/arquivo existente removido):**
  - `src/routes/privacidade.tsx` (novo): Política de Privacidade — descreve os dados efetivamente coletados (nome/e-mail/telefone/CPF de todos, localização de passageiro, CNH/documentos de veículo/localização em tempo real de motorista, dados de pagamento via Mercado Pago), para que são usados, compartilhamento com terceiros (Mercado Pago, Mapbox, link de viagem compartilhada), retenção/exclusão e os direitos do titular sob a LGPD.
  - `src/routes/termos.tsx` (novo): Termos de Uso — natureza de intermediação tecnológica do Zuvvi (motorista é autônomo, não empregado), regras de cadastro, cobrança/cancelamento Pix (referencia a trava da Microetapa 5.4), conduta esperada e limitação de responsabilidade.
  - `src/routes/index.tsx`: os três links do rodapé passam a apontar para `/privacidade`, `/termos` e `mailto:suporte@zuvvi.app` (endereço de suporte já usado como padrão em `web-push.server.ts`, não inventado agora) em vez de `href="#"`.
  - `src/routeTree.gen.ts`: regenerado (mesmo processo das etapas anteriores) para incluir as duas rotas novas.
- **Validação:** `npx tsc --noEmit` sem erros novos — inclusive dois erros pré-existentes em `privacidade.tsx`/`termos.tsx` (tipagem de `to=` antes do routeTree regenerado) desaparecem depois da regeneração, confirmando que a causa era só o routeTree desatualizado. `npx eslint --fix` aplicado só nos dois arquivos novos (quebras de linha do prettier), conteúdo inalterado; `index.tsx` e `routeTree.gen.ts` sem nenhum erro novo (comparado linha a linha contra o estado anterior).
- **NÃO é aconselhamento jurídico** — o texto foi escrito refletindo o que o código do projeto de fato faz (coleta e finalidade real dos dados), mas a validação final por um advogado/DPO antes do lançamento público continua sendo responsabilidade do responsável do projeto, especialmente se o Zuvvi vier a atuar em mais de um estado ou mudar o que coleta.

### Microetapa 5.12 — Recuperação de senha ("Esqueci minha senha") — ✅ IMPLEMENTADA
- **Achado:** o app não tinha nenhuma tela nem função de recuperação de senha. Um usuário cadastrado por e-mail/senha (não por Google) que esquecesse a senha ficava permanentemente travado fora da conta — bloqueador real de lançamento, não um nice-to-have.
- **Correção (estritamente aditiva, nada do fluxo de login existente foi alterado):**
  - `src/routes/auth.esqueci-senha.tsx` (novo): formulário de e-mail que chama `supabase.auth.resetPasswordForEmail(email, { redirectTo: .../auth/redefinir-senha })`. Sempre mostra a mesma tela de "verifique seu e-mail" independente do e-mail existir ou não na base, para não permitir enumeração de contas cadastradas.
  - `src/routes/auth.redefinir-senha.tsx` (novo): página que recebe o link do e-mail (fluxo PKCE do Supabase, `detectSessionInUrl: true` já configurado em `client.ts`), escuta `onAuthStateChange` por `PASSWORD_RECOVERY` (com fallback por `getSession()` e timeout de 4s caso o evento já tenha disparado antes da inscrição), mostra "link inválido/expirado" se a URL já chegar com `?error=...` ou se nada resolver a tempo, e só então libera o formulário de nova senha (mínimo 6 caracteres, confirmação obrigatória), que chama `supabase.auth.updateUser({ password })`.
  - `src/routes/auth.login.tsx`: adicionado o link "Esqueceu sua senha?" ao lado do campo de senha — única mudança no arquivo existente, resto do formulário de login intocado.
  - `src/routeTree.gen.ts`: regenerado para incluir as duas rotas novas.
- **Validação:** `npx tsc --noEmit` sem erros novos (os dois erros de `to=` antes de regenerar o routeTree desaparecem depois, mesmo padrão das etapas anteriores). `npx eslint` sem erros novos — o único erro nos arquivos novos (`redirectTo as any` em `auth.redefinir-senha.tsx`) é o mesmo padrão pré-existente já usado em `auth.login.tsx` linha 61 (o retorno de `resolvePostLoginDestination`/`resolveDestinationInternal` é `string` puro, não union de rotas literais), não uma dívida nova.
- **Ação necessária antes do deploy (config, não código):** no dashboard do Supabase, em Authentication → URL Configuration, `https://<domínio-de-produção>/auth/redefinir-senha` precisa estar na lista de Redirect URLs permitidas — sem isso o Supabase recusa o redirect do e-mail de recuperação. Authentication → Email Templates → "Reset Password" também deve estar habilitado (vem habilitado por padrão, mas vale confirmar antes do piloto).
- **NÃO testado neste sandbox:** o envio real do e-mail e o clique no link (dependem de SMTP configurado no projeto Supabase e de um navegador real) — mesma limitação de sempre. A lógica de estado (`verificando`/`pronto`/`invalido`) foi revisada linha a linha contra a documentação do fluxo PKCE do supabase-js v2, mas a prova de ponta a ponta fica para a homologação em ambiente real, junto com os outros itens já pendentes (Pix, push, upload).

## Limpeza de bugs — início 05/09/2026

Nova frente a pedido do usuário: em vez de auditoria de features faltando, uma varredura ponta a ponta atrás de **bugs reais e buracos de segurança** no que já existe, corrigidos em etapas pequenas e isoladas, cada uma validada antes de seguir para a próxima — sem alterar nada que já funcione.

### Limpeza 1 — Cancelamento de corrida travado quando a busca expira sem motorista — ✅ CORRIGIDA
- **Motivação:** usuário reportou print de uma corrida em dinheiro parada em "BUSCANDO MOTORISTA" onde apertar "Cancelar Corrida" não teve efeito.
- **Investigação:** revisão completa de `cancelarCorrida` (`src/lib/user.functions.ts`) e da tela `src/routes/procurando-motorista.tsx`. Para o cenário exato do print (status `buscando_motorista`, pagamento `dinheiro`), a lógica do servidor já permitia o cancelamento — confirmado reconstruindo o filtro `.or(...)` como SQL puro num Postgres local e comparando linha a linha contra 11 combinações de status/forma de pagamento. Não foi possível reproduzir com certeza absoluta o instante exato do print sem logs do aparelho, mas a investigação encontrou dois bugs reais e adjacentes na mesma tela, que batem com o sintoma relatado (a busca expirar sem que a tela do passageiro tenha acompanhado a tempo):
  1. `cancelarCorrida` não incluía o status `sem_motorista` (busca expirada sem nenhum motorista) na lista de status canceláveis. Se o servidor já tivesse marcado a corrida como `sem_motorista` no instante exato em que o passageiro clicasse em "Cancelar Corrida" (uma corrida de R$8 tem uma janela de busca curta), a chamada falhava com um erro genérico — exatamente o sintoma relatado.
  2. O botão "TENTAR NOVAMENTE" (que aparece quando a busca já expirou) nunca cancelava a corrida de verdade — só navegava de volta pra home, abandonando a corrida presa em `sem_motorista` para sempre, sem `cancelado_por`/`data_cancelamento` preenchidos (corrida "fantasma" nos relatórios).
- **Correção (mínima, escopo isolado):**
  - `src/lib/user.functions.ts`, `cancelarCorrida`: `sem_motorista` adicionado à lista de status sempre canceláveis (não depende de forma de pagamento — nesse status nunca existe cobrança Pix pendente, já que a cobrança só é criada depois do aceite de um motorista).
  - `src/routes/procurando-motorista.tsx`: o botão "TENTAR NOVAMENTE" agora chama `cancelarCorridaFn` (best-effort — se falhar, ainda assim navega para não prender o usuário) antes de voltar para a home, fechando a corrida corretamente.
- **Validação:** o novo filtro `.or(...)` foi reconstruído como SQL e testado contra uma tabela real (`corrida_status`/`forma_pagamento` como enums, 11 linhas cobrindo cada combinação relevante) num Postgres 16 local — confirma que `sem_motorista` fica sempre cancelável e que `aceita`/`motorista_a_caminho` com Pix continuam bloqueados exatamente como antes (nenhuma regressão na trava da Microetapa 5.4). `npx tsc --noEmit` idêntico antes/depois (zero erros novos). `npx eslint` com a mesma contagem exata de problemas antes/depois (164, sendo 163 erros pré-existentes de formatação/aspas neste arquivo que nunca passou por `prettier --write` — não uma dívida nova introduzida agora).
- **Ainda não confirmado:** se o print original era exatamente essa corrida de race, ou outra causa (ex.: falha de rede pontual no aparelho) — sem acesso a logs do dispositivo não dá para provar com certeza qual dos dois. Se o problema persistir depois desta correção, o próximo passo é capturar o console do navegador no momento do clique.

### Limpeza 2 — Código de embarque sem limite de tentativas (força bruta) — ✅ CORRIGIDA
- **Achado:** revisando os fluxos adjacentes ao cancelamento (Limpeza 1), `iniciarCorrida` (`src/lib/motorista.functions.ts`) comparava o `codigo_embarque` (4 dígitos, só 10 mil combinações) sem NENHUM limite de tentativas. Um motorista já atribuído à corrida (não precisa de acesso especial, só ter aceitado a corrida) podia, em tese, script automatizado na mão, tentar as 10 mil combinações e iniciar a corrida — e começar a cobrar — sem o passageiro ter validado o embarque de verdade. Buraco de segurança real, independente do bug de cancelamento.
- **Correção:**
  - Nova migration `20260905140000_limitar_tentativas_codigo_embarque.sql`: coluna `tentativas_codigo_embarque` em `corridas` (smallint, default 0, `add column if not exists` — idempotente, não quebra linhas existentes).
  - `iniciarCorrida`: antes de comparar o código, bloqueia se `tentativas_codigo_embarque >= 5` ("Muitas tentativas incorretas..."); a cada tentativa errada, incrementa a coluna atomicamente via UPDATE e informa quantas tentativas restam. 5 chances é suficiente para um passageiro real ditar o código errado por engano, mas reduz a superfície de força bruta de 10.000 tentativas para 5.
- **Validação:** `npx tsc --noEmit` idêntico antes/depois (mesmos 2 erros pré-existentes, só linha deslocada). `npx eslint`: zero erros novos da classe `no-explicit-any` (tipei o resultado do select com um type local `CorridaCodigoEmbarque` em vez de `as any`); o total de problemas subiu de 245 para 250, mas só por formatação (`prettier/prettier`) nas linhas que adicionei — este arquivo nunca passou por `prettier --write` (245 erros de formatação já eram pré-existentes antes desta etapa), então isso não é dívida de tipo nova, só a mesma dívida de estilo já existente se estendendo às minhas linhas novas. Lógica do incremento/limite testada num Postgres 16 local: 5 tentativas erradas seguidas levam a coluna a 5, e a trava dispara exatamente no limite certo.
- **Efeito colateral aceito:** um passageiro que erra o código de verdade (dedo trêmulo, letra ilegível) agora tem só 5 chances em vez de infinitas — julgo a troca correta dado o risco de segurança, mas é uma mudança de comportamento visível, registrada aqui para o caso de gerar reclamação de suporte.
- **CORREÇÃO DE CI (mesmo commit da Limpeza 2, logo em seguida):** o push original quebrou 2 checks (`bunx tsc --noEmit` real, rodado pelo CI) com `TS2379` — meu tipo local `CorridaCodigoEmbarque` (com `status: string`) não é compatível com o tipo estrito gerado pelo Supabase para `.update()` (`exactOptionalPropertyTypes` + `RejectExcessProperties`, que exige o enum exato de `status`). **Achado importante sobre o ambiente:** este sandbox não tem `@supabase/*` instalado em `node_modules` (bloqueado pela mesma restrição de registry já documentada nesta sessão) — então todo `npx tsc --noEmit` rodado localmente nesta sessão inteira infere `any` para `supabaseAdmin` e **nunca pega esse tipo de erro**, só o CI real pega. Corrigido trocando os tipos locais por `as any` no `.select()`/`.update()`, igual ao padrão já usado em todo o resto do arquivo (`cancelarCorridaMotorista`, `marcarMotoristaACaminho`, etc.) — que é justamente o padrão que não quebra, porque não tenta checar a forma exata do objeto contra o tipo gerado. Registro isso para não repetir esse erro específico em etapas futuras: **evitar tipos locais customizados em argumentos de `.update()`/`.insert()` do Supabase nesta sandbox — usar `as any` como o resto do arquivo já faz, e deixar o CI real como a única validação de tipo confiável para código que toca `supabaseAdmin`.**

### Limpeza 3 — Limite de contatos de confiança sem trava atômica (condição de corrida) — ✅ CORRIGIDA
- **Achado:** revisando o código de Contatos de Confiança (Microetapa 5.8) em busca de mais bugs adjacentes, `criarContatoConfianca` (`src/lib/contatos-confianca.functions.ts`) checava o limite de 5 contatos com um `COUNT` antes do `INSERT` — sem nenhuma trava atômica. Duas requisições concorrentes do mesmo passageiro podiam passar as duas pela checagem antes de qualquer uma inserir, resultando em mais de 5 contatos salvos (baixo risco, mas um bug real de corrida). O padrão idêntico já tinha acontecido antes com Favoritos (limite de 10) e foi corrigido lá com um trigger + `pg_advisory_xact_lock` (migration `20260821212540`) — só não tinha sido replicado para Contatos de Confiança quando essa feature foi criada.
- **Correção:** nova migration `20260905150000_enforce_contatos_confianca_limit.sql`, réplica exata da técnica já usada em Favoritos: trigger `BEFORE INSERT` que toma um `pg_advisory_xact_lock` por `passageiro_id` (serializa inserções concorrentes do mesmo usuário, sem travar outros usuários) e rejeita com `ERRCODE 23514` se já houver 5 contatos. `criarContatoConfianca` mantém a checagem rápida em código (bom UX no caso comum) e agora também traduz o erro `23514` do banco para a mesma mensagem amigável — a garantia real passa a ser o trigger, não mais só a checagem de aplicação.
...
## Limpeza 4 — Tela "crua" por vários segundos no primeiro carregamento — ✅ CORRIGIDA

- **Motivação:** usuário mandou 2 prints do app logo após carregar — o primeiro mostra o layout completamente sem estilo (header pequeno e mal posicionado, botões sem formatação, tudo espremido/duplicado), o segundo (uns 7 segundos depois) mostra o layout correto normal.
- **Diagnóstico:** essa assinatura exata (tudo cru/sem estilo por vários segundos, depois "estala" pro layout certo) é a marca registrada de uma folha de estilo de terceiro travando a primeira pintura da página inteira. O `<head>` do app (`src/routes/__root.tsx`) carregava a fonte Poppins via `<link rel="stylesheet" href="https://fonts.googleapis.com/...">` — um recurso de outro domínio que, numa conexão mais lenta (rede móvel, primeiro acesso sem nada em cache), pode demorar vários segundos pra responder. Enquanto isso, o navegador não pinta nada com estilo — daí a tela "crua".
- **Tentativa descartada:** cheguei a tentar o truque clássico (`media="print"` + trocar pra `"all"` no `onload`), mas descobri que o `<HeadContent>` do TanStack Router renderiza esses links como elementos `<link>` React de verdade (com gerenciamento nativo de "precedence" do React 19) — passar uma string pro prop `onLoad` não funciona nesse sistema (React espera uma função, não serializa pro HTML como atributo `onload`). Reverti antes de commitar por não conseguir validar que funcionaria de verdade.
- **Correção efetiva:** a fonte Poppins (pesos 400/500/600/700, subset "latin" — cobre todos os acentos do português) agora é servida do próprio domínio (`public/fonts/*.woff2`, ~8KB cada, baixados diretamente do Google Fonts e versionados no repo) via `@font-face` em `src/styles.css`, em vez de um `<link>` externo. Isso elimina por completo a dependência de rede de terceiro que causava o travamento — o arquivo carrega junto com o resto dos assets estáticos do próprio app, sem round-trip de DNS/TLS pra outro domínio.
- `src/routes/__root.tsx`: removidos os 2 `<link rel="preconnect">` e o `<link rel="stylesheet">` do Google Fonts, que não são mais necessários.
- **Validação:** confirmei os 4 arquivos `.woff2` como válidos (`file` reconhece o formato) e que `--font-sans: "Poppins", ...` em `styles.css` já referenciava exatamente esse `font-family`, então nenhuma outra mudança é necessária pro resto do app continuar usando a mesma fonte. CSS validado por chaves balanceadas (41 aberturas/41 fechamentos). `npx tsc --noEmit` e `npx eslint`: zero erros novos em `__root.tsx` (só os 2 erros pré-existentes de `any`, na mesma linha de sempre). Não consegui rodar `vite build` neste sandbox pra confirmar o bundle final (`vite-plugin-pwa` não está instalado aqui, mesma restrição de registry já documentada) — a validação ficou nos arquivos-fonte e na lógica CSS/HTML, não no build de produção completo.
- **Não confirmado:** se o Google Fonts realmente estava lento no aparelho do usuário (não tenho como medir isso à distância) — mas essa é a causa mais conhecida e documentada desse sintoma exato, e a correção (servir a fonte localmente) é estritamente uma melhoria de robustez independente da causa raiz real ser essa ou não: menos uma dependência de rede externa é sempre menos um jeito de travar.

## Limpeza 5 — Botão "Carteira" do passageiro sem nenhuma ação — ✅ CORRIGIDA

- **Achado:** usuário perguntou o que o ícone "Carteira" no menu inferior da tela inicial deveria mostrar. Investigando, o botão era um `<button>` sem `onClick` e sem navegação — diferente de "Corridas" e "Perfil" (que já eram `<Link>` funcionais). Não existia nenhuma tela de carteira do lado do passageiro; só o motorista tinha uma (Microetapa 5.9, ganhos).
- **Correção (mesmo padrão da carteira do motorista, adaptado pra gasto em vez de ganho):**
  - `src/lib/carteira-passageiro.functions.ts` (novo): `getCarteiraPassageiro` — mesma lógica de agregação hoje/semana/mês/total (fuso `America/Sao_Paulo`), mas somando `valor_total` de corridas concluídas com pagamento `pago`, filtrando por `passageiro_id` em vez de `motorista_id`. Não expõe `valor_comissao`/`valor_motorista` — o passageiro não precisa ver o split interno da plataforma.
  - `src/routes/carteira-passageiro.tsx` (novo): mesma UI da carteira do motorista (card de resumo, grid hoje/7 dias/mês, lista das últimas corridas pagas), com o guard de rota exclusivo de passageiro já usado em `corridas.tsx` (`resolveDestinationForLoader` + `isPassageiro === true && redirectTo === "/" && !isAdmin && !isMotorista`).
  - `src/routes/index.tsx`: o `<button>` inerte de "Carteira" virou `<Link to="/carteira-passageiro">`, mesma classe já usada pelos links "Corridas"/"Perfil".
  - `src/routeTree.gen.ts`: regenerado.
- **Validação:** `npx tsc --noEmit` idêntico antes/depois (só o erro esperado de `to=` desaparece depois de regenerar o routeTree, mesmo padrão de sempre). `npx eslint`: `carteira-passageiro.functions.ts` sem nenhum erro; `carteira-passageiro.tsx` só tem o mesmo `as any` já usado em `corridas.tsx` pro mesmo motivo (`redirectTo` é `string` puro); `index.tsx` sem categoria nova de erro, só a mesma dívida de formatação pré-existente se estendendo à linha nova.
- **Não testado neste sandbox:** navegação real num navegador (sem acesso a browser aqui) — validação ficou em tipo/lint, igual às etapas anteriores.

## Limpeza 6 — Cobertura de teste incompleta e teste desatualizado no estado "falhou" do Pix — ✅ CORRIGIDA

- **Contexto:** com o app estável e o PR sem pendência aberta, escolhi (autorizado pelo usuário a escolher o próximo passo) reforçar testes automatizados como o item de menor risco possível — só adiciona cobertura, não toca em nenhum comportamento existente. O alvo foi `src/lib/pagamento-pix-status.functions.ts` (`derivarEstadoPagamentoPix`, `getPixPaymentTimeoutSeconds`), a máquina de estados que decide o que a tela de pagamento Pix mostra — a área mais crítica financeiramente do app.
- **Achado real durante a investigação (não relacionado à minha mudança):** `scripts/pix/pagamento-pix.test.ts` já existia e cobria boa parte da cascata, mas 3 das suas próprias asserções sobre o texto da tela de "falhou" estavam **desatualizadas** em relação ao `src/routes/pagamento-pix.tsx` atual — o teste esperava o texto antigo ("Não foi possível concluir o pagamento Pix" / botão "Tentar novamente"), enquanto o código real já mostra um texto mais claro ("Pagamento Pix não concluído" / botão "Escolher outro pagamento"). Confirmei que essa quebra já existia **antes** desta sessão tocar o arquivo (rodei o teste original via `git stash`, e ele já falhava do mesmo jeito) — não foi algo que eu quebrei. Esse teste específico só roda no workflow `pix-pagamento-passageiro.yml`, que é restrito a uma branch fixa (`feature/pix-100-seguro`) e nunca executou nesta PR, por isso a divergência nunca apareceu num CI vermelho.
- **Correção:** atualizei as 3 asserções de texto pra bater com o comportamento real e já correto da tela (nenhuma mudança de comportamento, só o teste passou a descrever a verdade).
- **Cobertura nova adicionada (mesmo arquivo, só amplia o que já existia):**
  - Limites inclusivos do clamp de timeout (`60` e `900` segundos devem passar como estão, não cair no default de 300).
  - Estorno detectado só pela tentativa (`tentativaEstado === "estornado"`), mesmo com `pagamentoStatus` ainda `"pendente"` — cenário de webhook chegando antes da tabela local ser atualizada.
  - Provedor recusando (`rejected`) ou cancelando (`cancelled`) a cobrança cai em `"falhou"`, mesmo sem o app ter marcado assim.
  - Tentativa registrada mas ainda sem `pixCopiaCola` continua `"gerando"`, nunca `"aguardando"` (evita mostrar um QR code vazio).
  - `tentativaEstado === "pago"` isolado (antes do provedor confirmar) e `providerStatus === "in_mediation"` — ambos levam a `"analisando"`, nunca liberando a corrida sozinhos.
- **Validação — desta vez com execução real, não só leitura manual:** o sandbox não tem `@supabase/supabase-js` instalado (limitação de registry já documentada nesta sessão), o que impedia rodar o arquivo de teste direto (falha ao resolver o import estático em `auth-middleware.ts`, usado só pelo `createServerFn`, nunca pelas funções puras testadas). Criei um stub mínimo e temporário em `node_modules/@supabase/supabase-js` (pasta ignorada pelo git, nunca chega a ser commitada) só pra satisfazer a resolução de módulo do Bun, rodei `bun scripts/pix/pagamento-pix.test.ts` de verdade (todas as asserções, antigas e novas, passaram), depois apaguei o stub. `npx prettier --check` e `npx eslint` no arquivo: sem nenhum erro.
- **Decisão consciente:** não criei/editei nenhum workflow de CI nesta etapa — o `pix-pagamento-passageiro.yml` já cobre esse arquivo de teste (mesmo que só rode numa branch fixa, fora do meu controle), e criar um novo workflow genérico pra rodar `scripts/pix/*.test.ts` em qualquer PR seria uma mudança de infraestrutura de CI maior, fora do escopo de "sem tocar no que já está funcional".
