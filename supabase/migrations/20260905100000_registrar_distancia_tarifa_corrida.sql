-- G3: a corrida não guardava distância, tempo nem a tarifa efetivamente
-- aplicada na cotação — valor_final sempre reproduzia valor_estimado sem
-- nenhum rastro do cálculo que o originou. Estas colunas congelam o que foi
-- cotado (e assinado via HMAC em cotarCorrida) no momento da criação.
alter table public.corridas
  add column if not exists distancia_km numeric,
  add column if not exists duracao_min numeric,
  add column if not exists tarifa_bandeirada numeric,
  add column if not exists tarifa_valor_km numeric,
  add column if not exists tarifa_valor_min numeric,
  add column if not exists tarifa_minima numeric;

comment on column public.corridas.distancia_km is
  'Distância da rota calculada pelo Mapbox no momento da cotação (cotarCorrida), em km.';
comment on column public.corridas.duracao_min is
  'Duração estimada da rota no momento da cotação, em minutos.';
comment on column public.corridas.tarifa_bandeirada is
  'Valor de bandeirada da cidade efetivamente usado para calcular valor_estimado.';
comment on column public.corridas.tarifa_valor_km is
  'Valor por km da cidade efetivamente usado para calcular valor_estimado.';
comment on column public.corridas.tarifa_valor_min is
  'Valor por minuto da cidade efetivamente usado para calcular valor_estimado.';
...
create or replace function public.criar_corrida_financeira_atomica(
  ...
  p_tarifa_bandeirada numeric,
  p_tarifa_valor_km numeric,
  p_tarifa_valor_min numeric,
  p_tarifa_minima numeric
)
returns uuid
language plpgsql
security invoker
