-- Expõe na tela pública de viagem compartilhada os horários reais de cada
-- etapa da corrida (já existentes em public.corridas), pra montar uma linha
-- do tempo em vez de só mostrar a etapa atual. Não altera nenhum campo
-- existente, join existente, política de segurança, máquina de estados,
-- localização, SOS ou qualquer outra regra da corrida.
--
...
-- 20260915220000, 20260917090000): drop + create dentro da mesma transação,
-- sem janela de indisponibilidade pra chamadores concorrentes.
...
comment on function public.get_viagem_compartilhada_publica is
  'Leitura pública e minimalista de uma viagem compartilhada, incluindo referências de foto do motorista/passageiro e horários das etapas da corrida.';
