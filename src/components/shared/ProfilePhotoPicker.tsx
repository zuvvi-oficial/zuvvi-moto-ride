...
export type ProfilePhotoPickerProps = {
  getPhotoFn: () => Promise<{ path: string | null; signedUrl: string | null }>;
  savePhotoFn: (opts: { data: { path: string } }) => Promise<{ success: boolean; path: string }>;
  queryKey: QueryKey;
  // Diâmetro do avatar em pixels — os dois usos (passageiro e motorista)
  // ficam em layouts diferentes (um centralizado, outro numa linha ao lado
  // do nome), então o tamanho não é fixo.
  sizePx?: number;
};

// Seletor de foto de perfil compartilhado entre passageiro e motorista: os
// dois guardam a foto no mesmo bucket/coluna (usuarios.foto_perfil_path,
// bucket fotos-perfil), só o server function de leitura/gravação muda
// conforme o papel — por isso a UI é uma única implementação parametrizada.
export function ProfilePhotoPicker({
  ...
  const handleSelectedFile = async (file?: File) => {
    ...
    try {
      ...
      toast.error(message || "Não foi possível atualizar sua foto.");
    } finally {
    ...
    <>
      <div className="relative shrink-0" style={{ width: sizePx, height: sizePx }}>
        <button
          ...
          className="relative h-full w-full rounded-full overflow-hidden bg-zuvvi-volt/10 flex items-center justify-center border-2 border-zuvvi-volt/20 transition-all active:scale-[0.98] disabled:opacity-70"
        >
          {photoUrl ? (
            <img src={photoUrl} alt="Foto de perfil" className="h-full w-full object-cover" />
          ) : isLoading ? (
            <Loader2 className="w-1/3 h-1/3 text-zuvvi-volt animate-spin" />
          ) : (
            <User className="w-1/2 h-1/2 text-zuvvi-volt" />
          )}
        ...
        <button
          ...
          className="absolute -right-1 -bottom-1 h-8 w-8 rounded-full bg-zuvvi-volt text-zuvvi-indigo border-4 border-zuvvi-indigo-dark flex items-center justify-center shadow-lg active:scale-95 disabled:opacity-70"
        >
          ...
          <div className="grid gap-3 pt-2">
            ...
            >
              ...
              <span>
                ...
                <span className="block text-[11px] text-muted-foreground">
                  Usar uma foto que já está no celular
                </span>
              </span>
            </button>
            ...
            >
              ...
              <span>
                ...
                <span className="block text-[11px] text-muted-foreground">
                  Tirar uma foto de perfil agora
                </span>
              </span>
            </button>
          </div>
