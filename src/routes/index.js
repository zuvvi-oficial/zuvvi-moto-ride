...
import { User, MapPin, Clock, Star, Bike, CreditCard, LogOut, ChevronRight, LocateFixed, AlertTriangle, Loader2 } from "lucide-react";
...
function UnifiedIndex() {
    ...
    if (isLoading) {
        return (<div className="min-h-screen bg-zuvvi-indigo-dark flex items-center justify-center">
...
function HomePassageiro({ nome }) {
    ...
    const [isCityAvailable, setIsCityAvailable] = useState(null);
    const [cityName, setCityName] = useState(null);
    const [debugStatus, setDebugStatus] = useState("Verificando token...");
    const [availabilityError, setAvailabilityError] = useState(null);
    const [originAddress, setOriginAddress] = useState("Buscando endereço...");
    ...
    const requestLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition((position) => {
            ...
            if (map.current) {
                ...
                const existingMarker = map.current._zuvviMarker;
                if (existingMarker) {
    ...
    const initMap = async (container) => {
        ...
        try {
            ...
            map.current._zuvviMarker = marker;
        }
        catch (err) {
    ...
    return (<div className="relative bg-zuvvi-indigo text-foreground overflow-hidden" style={{ height: '100dvh', width: '100vw' }}>
      {/* 1. O Mapa (Z-INDEX 0) - Camada de Fundo */}
      <div ref={mapContainerRef} style={{ position: 'absolute', inset: 0, zIndex: 0 }} className={`transition-opacity duration-1000 ${location ? 'opacity-100' : 'opacity-0'}`}/>

      {/* 2. Camada de Interface (Z-INDEX 10) - Sobreposta ao mapa */}
      <div className="absolute inset-0 z-10 flex flex-col pointer-events-none overflow-y-auto" style={{ height: '100dvh' }}>
        ...
        <div className="absolute top-2 right-2 z-[9999] bg-black/90 text-white text-[10px] px-3 py-2 rounded-lg border border-white/20 pointer-events-none uppercase tracking-tighter flex flex-col gap-1 shadow-2xl backdrop-blur-md min-w-[200px]">
          ...
          <div className="flex flex-col gap-0.5 opacity-80">
            ...
            {availabilityError && (<p className="text-red-400 normal-case mt-1 bg-red-500/10 p-1 rounded border border-red-500/20">
        ...
        <header className="px-5 py-4 pointer-events-auto shrink-0">
          <div className="mx-auto max-w-md flex items-center justify-between bg-zuvvi-indigo/60 backdrop-blur-lg border border-white/10 rounded-3xl px-4 py-3 shadow-2xl">
            ...
            <button onClick={handleLogout} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center border border-white/10 transition-colors hover:bg-white/10" title="Sair">
        ...
        <main className="flex-1 flex flex-col justify-end px-5 pb-28 mx-auto w-full max-w-md space-y-4">
          ...
          {isLocating && (<div className="bg-zuvvi-indigo/90 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-10 flex flex-col items-center justify-center text-center space-y-4 shadow-2xl pointer-events-auto animate-rise">
              <Loader2 className="w-10 h-10 text-zuvvi-volt animate-spin"/>
              <p className="text-sm font-medium">Buscando sua localização...</p>
            </div>)}

          {!isLocating && locationError && (<div className="bg-zuvvi-indigo/90 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 flex flex-col items-center justify-center text-center space-y-6 shadow-2xl pointer-events-auto animate-rise">
              ...
              <button onClick={requestLocation} className="w-full bg-zuvvi-volt text-zuvvi-indigo py-4 rounded-2xl font-black uppercase tracking-widest text-xs zuvvi-glow transition-transform active:scale-95">
                TENTAR NOVAMENTE
              </button>
            </div>)}

          {!isLocating && !locationError && isCityAvailable === false && (<div className="bg-zuvvi-indigo/90 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 flex flex-col items-center justify-center text-center space-y-6 shadow-2xl pointer-events-auto animate-rise">
          ...
          {!isLocating && !locationError && isCityAvailable === true && (<div className="space-y-4 animate-rise pointer-events-auto">
              {/* Card de Origem */}
              <div className="bg-zuvvi-indigo/90 backdrop-blur-xl border border-white/10 rounded-[2rem] p-4 shadow-2xl space-y-3">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setIsEditingOrigin(!isEditingOrigin)}>
                  ...
                  <button onClick={requestLocation} disabled={isUpdatingLocation} className={`w-10 h-10 rounded-full bg-white/5 flex items-center justify-center border border-white/10 transition-all hover:bg-white/10 active:scale-95 ${isUpdatingLocation ? 'opacity-50' : ''}`}>
                    <LocateFixed className={`w-4 h-4 text-zuvvi-volt ${isUpdatingLocation ? 'animate-spin' : ''}`}/>
                  </button>
                </div>

                {isEditingOrigin && (<div className="pt-2 border-t border-white/5 animate-in fade-in slide-in-from-top-2 duration-300">
                    <DestinoSearch location={location} placeholder="Pesquisar nova origem..." autoFocus={true} onSelect={(res) => {
              ...
              {/* Card de Destino */}
              <DestinoSearch location={isManualOrigin ? manualLocation : location} onSelect={(dest) => {
...
function DestinoSearch({ location, onSelect, placeholder = "Para onde vamos?", autoFocus = false }) {
    ...
    return (<div className="relative group">
      ...
      <input type="text" value={query} autoFocus={autoFocus} onChange={(e) => setQuery(e.target.value)} placeholder={placeholder} className="w-full bg-zuvvi-indigo/90 backdrop-blur-xl border border-white/10 rounded-[2rem] py-6 pl-14 pr-4 focus:ring-2 focus:ring-zuvvi-volt/50 focus:border-zuvvi-volt outline-none transition-all shadow-2xl text-base font-bold placeholder:text-muted-foreground/50"/>
      
      {isOpen && results.length > 0 && (<div className="absolute top-full left-0 right-0 mt-3 bg-zuvvi-indigo/95 backdrop-blur-2xl border border-white/10 rounded-[2rem] shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-300">
...
function LandingPage() {
    return (<div className="min-h-screen zuvvi-gradient text-foreground">
      {/* Nav */}
      <header className="sticky top-0 z-40 border-b border-border/60 bg-background/80 backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/auth/cadastro" className="rounded-full bg-zuvvi-volt px-6 py-2 text-xs font-bold text-zuvvi-indigo shadow-lg shadow-zuvvi-volt/20 hover:scale-[1.02] transition-transform">
              CADASTRAR
            </Link>
          </div>
        </div>
      ...
      <section className="relative overflow-hidden px-5 py-20 text-center lg:py-32">
        ...
        <div className="mt-20 max-w-5xl mx-auto rounded-[2rem] overflow-hidden border border-border/80 zuvvi-glow relative">
          <img src={heroMoto} alt="Mototaxista Zuvvi" className="w-full aspect-[21/9] object-cover opacity-90"/>
