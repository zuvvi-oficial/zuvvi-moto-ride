...
import { Eye, Clock, User, FileText, Bike, History, ExternalLink, MapPin, XCircle, AlertTriangle, Settings2, LogOut } from 'lucide-react';
...
function AdminVeiculos() {
    ...
    const [viewingVeiculoId, setViewingVeiculoId] = useState(null);
    ...
    const handleUpdateCampo = async (campo) => {
        if (!viewingVeiculoId)
            return;
        try {
    ...
    return (<div className="min-h-screen bg-zuvvi-indigo text-white flex flex-col">
      ...
      <div className="hidden md:block rounded-md border border-white/10">
        <Table>
          ...
          <TableBody>
            {veiculos?.map((veiculo) => (<TableRow key={veiculo.id} className="border-white/10 hover:bg-white/5">
                ...
                <TableCell>
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${veiculo.status_aprovacao === 'aprovado' ? 'bg-green-500/20 text-green-500' :
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button variant="ghost" size="icon" className="hover:bg-white/10 text-volt" onClick={() => setViewingVeiculoId(veiculo.id)}>
                ...
                <TableCell className="text-right space-x-2">
                  {veiculo.status_aprovacao !== 'aprovado' && (<Button size="sm" className="bg-green-600 hover:bg-green-700" onClick={() => {
                ...
                setActionType('recusado');
                  ...
                  {veiculo.status_aprovacao === 'aprovado' && (<Button size="sm" variant="outline" className="border-orange-500 text-orange-500 hover:bg-orange-500/10" onClick={() => {
                    setSelectedVeiculo(veiculo);
      ...
      <div className="md:hidden space-y-4">
        {veiculos?.map((veiculo) => (<div key={veiculo.id} className="bg-white/5 border border-white/10 rounded-[1.5rem] p-5 space-y-5">
            <div className="flex justify-between items-start">
              ...
              <Badge className={cn("rounded-lg px-2 py-0.5 text-[10px] font-black uppercase tracking-widest", veiculo.status_aprovacao === 'aprovado' ? 'bg-green-500/20 text-green-500 border-green-500/20' :
            ...
            <div className="flex flex-col gap-2">
              <Button variant="outline" className="w-full h-12 rounded-xl border-white/10 hover:bg-white/5 font-bold uppercase text-[10px] tracking-widest flex items-center justify-center gap-2" onClick={() => setViewingVeiculoId(veiculo.id)}>
                <Eye className="w-4 h-4"/>
                Ver Detalhes
              </Button>
              
              <div className="flex gap-2">
                {veiculo.status_aprovacao !== 'aprovado' && (<Button className="flex-1 h-12 rounded-xl bg-green-600 hover:bg-green-700 font-bold uppercase text-[10px] tracking-widest" onClick={() => {
                    setSelectedVeiculo(veiculo);
                    setActionType('aprovado');
                }}>
                    Aprovar
                  </Button>)}
                {veiculo.status_aprovacao === 'aprovado' && (<Button variant="outline" className="flex-1 h-12 rounded-xl border-orange-500 text-orange-500 hover:bg-orange-500/10 font-bold uppercase text-[10px] tracking-widest" onClick={() => {
                    setSelectedVeiculo(veiculo);
                    setActionType('suspenso');
                }}>
                    Suspender
                  </Button>)}
                <Button variant="destructive" className="flex-1 h-12 rounded-xl bg-red-600 hover:bg-red-700 font-bold uppercase text-[10px] tracking-widest" onClick={() => {
                setSelectedVeiculo(veiculo);
                setActionType('recusado');
            }}>
                  Recusar
        ...
        <DialogContent className="bg-zuvvi-indigo border-white/10 text-white">
          ...
          <div className="space-y-4 py-4">
            ...
            {actionType !== 'aprovado' && (<div className="space-y-2">
                ...
                <Textarea className="bg-white/5 border-white/10 text-white" value={justificativa} onChange={(e) => setJustificativa(e.target.value)} placeholder={actionType === 'suspenso' ? "Informe o motivo da suspensão..." : "Informe o motivo da recusa..."}/>
              </div>)}
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setSelectedVeiculo(null)}>Cancelar</Button>
            <Button className={actionType === 'aprovado' ? "bg-green-600 hover:bg-green-700" : actionType === 'suspenso' ? "bg-orange-600 hover:bg-orange-700" : "bg-red-600 hover:bg-red-700"} disabled={actionType !== 'aprovado' && !justificativa} onClick={handleAction}>
              Confirmar
            </Button>
          </DialogFooter>
        </DialogContent>
          ...
          <ScrollArea className="h-[calc(100vh-8rem)] p-6">
            {loadingDetalhe ? (<div className="flex flex-col items-center justify-center py-12 space-y-4">
                ...
                <section className="space-y-4">
                  ...
                  {['recusado', 'suspenso'].includes(detalhe.veiculo.status_aprovacao) && (<div className="bg-orange-500/10 border border-orange-500/30 p-3 rounded-md flex gap-2 items-center text-orange-500 text-xs">
                ...
                { id: 'cor', label: 'Cor', type: 'text' },
                        ...
                        <div className="flex gap-2">
                          <Input id={field.id} type={field.type} className="bg-white/5 border-white/10 text-white flex-1" value={editData[field.id]} onChange={(e) => setEditData({ ...editData, [field.id]: e.target.value })} disabled={['recusado', 'suspenso'].includes(detalhe.veiculo.status_aprovacao)}/>
                          <Button size="icon" variant="secondary" className="bg-volt text-black hover:bg-volt/80 shrink-0" onClick={() => handleUpdateCampo(field.id)} disabled={['recusado', 'suspenso'].includes(detalhe.veiculo.status_aprovacao) || editData[field.id] === detalhe.veiculo[field.id]}>
                ...
                <section className="space-y-4">
                  ...
                  {detalhe.veiculo.motoristas ? (<div className="bg-white/5 p-4 rounded border border-white/10 space-y-3">
                      <div className="flex justify-between items-start">
                        <div>
                        ...
                        <Badge variant="outline" className={detalhe.veiculo.motoristas.status_aprovacao === 'aprovado' ? 'text-green-500 border-green-500/30' : 'text-amber-500 border-amber-500/30'}>
                    ...
                    </div>) : (<div className="text-sm text-gray-400 italic">Nenhum motorista vinculado.</div>)}
                </section>
                ...
                return (<Card key={tipo} className="bg-white/5 border-white/10 text-white">
                          <CardContent className="p-4 flex items-center justify-between">
                            <div className="space-y-1">
                              ...
                              <div className="flex items-center gap-2">
                                {doc ? (<>
                                    <Badge variant="outline" className={doc.status_analise === 'aprovado' ? 'text-green-500 border-green-500/30' :
                            ...
                            {doc?.publicUrl && (<Button variant="outline" size="sm" className="border-white/20 hover:bg-white/10" onClick={() => {
                            if (doc.publicUrl) {
