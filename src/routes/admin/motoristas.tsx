...
import {
  ...
  getMotoristaDetalheAdmin,
  updateStatusDocumento,
  getDocumentoUrlSigned,
...
const motoristasOptions = (filters: { status?: string; busca?: string }) =>
  queryOptions({
...
function AdminMotoristas() {
  ...
  const [isViewingFile, setIsViewingFile] = useState<{
    url: string;
    type: string;
    isPdf: boolean;
  } | null>(null);
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      toast.success(
        `Motorista ${actionType === "aprovado" ? "aprovado" : "atualizado"} com sucesso!`,
      );
    ...
    try {
      const result = await updateStatusDocFn({
        data: {
          ...
          novoStatus: status,
          justificativa:
            status === "recusado" || status === "correcao_solicitada"
              ? justificativaDoc
              : undefined,
        },
      });
  ...
  return (
    <div className="min-h-screen bg-zuvvi-indigo text-white flex flex-col">
      <AdminHeader />
      <AdminBottomNav isHidden={!!viewingMotoristaId} />
      ...
      <div className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 pb-24 md:pb-6">
            ...
            <TableBody>
              {motoristas?.map((user: any) => (
                <TableRow key={user.id} className="border-white/10 hover:bg-white/5">
                  ...
                  <TableCell>
                    {user.cidades?.nome}/{user.cidades?.estado_uf}
                  </TableCell>
                  <TableCell>
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold ${
                        user.motoristas.status_aprovacao === "aprovado"
                          ...
                          : user.motoristas.status_aprovacao === "em_analise"
                  ...
                  <TableCell>
                    <div
                      className={`h-2 w-2 rounded-full ${user.motoristas.is_disponivel ? "bg-volt" : "bg-gray-500"}`}
                    />
                  </TableCell>
                  ...
                  <TableCell className="text-right space-x-2">
                    ...
                    {user.motoristas.status_aprovacao !== "recusado" &&
                      user.motoristas.status_aprovacao !== "suspenso" && (
                        <Button
                          size="sm"
                          variant="destructive"
                          onClick={() => {
              ...
              <div className="flex justify-between items-start">
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      className={`h-2 w-2 rounded-full ${user.motoristas.is_disponivel ? "bg-volt shadow-[0_0_8px_rgba(198,255,61,0.5)]" : "bg-gray-500"}`}
                    />
        ...
        >
          <DialogContent className="bg-zuvvi-indigo border-white/10 text-white">
            ...
            <div className="space-y-4 py-4">
              ...
              {actionError && (
                <div
                  className="p-3 rounded border border-red-500/50 bg-red-500/10 text-red-200 text-sm"
                  role="alert"
                  aria-live="assertive"
                >
                  {actionError}
        ...
        <Sheet
          open={!!viewingMotoristaId}
          onOpenChange={(open) => !open && setViewingMotoristaId(null)}
        >
          <SheetContent
            side="right"
              ...
              {loadingDetalhe ? (
                ...
                <div className="space-y-8 pb-12">
                        ...
                        <CardContent className="pt-6 flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-xs text-green-200/70 mt-1">
                              Este motorista já possui aprovação ativa no sistema.
                            </p>
                          </div>
                      ...
                      <Card className="bg-white/5 border-white/10 text-white">
                        <CardContent className="pt-6 space-y-4">
                          {(() => {
                            ...
                            const docsAprovados = detalhe.documentos.filter(
                              (d: any) =>
                                tiposObrigatorios.includes(d.tipo_documento) &&
                                d.status_analise === "aprovado",
                            );

                            const hoje = getHojeBR();
                            const cnhValida =
                              detalhe.motorista.cnh_numero &&
                              (detalhe.motorista.cnh_categoria === "A" ||
                                detalhe.motorista.cnh_categoria === "AB") &&
                              detalhe.motorista.cnh_validade &&
                              detalhe.motorista.cnh_validade >= hoje;

                            const veiculoAprovado =
                              detalhe.veiculo && detalhe.veiculo.status_aprovacao === "aprovado";
                            const docsCompletos = docsAprovados.length === tiposObrigatorios.length;

                            const prontoParaAprovar =
                              detalhe.motorista.status_aprovacao === "em_analise" &&
                              veiculoAprovado &&
                              cnhValida &&
                              docsCompletos;
                              ...
                              return (
                                <div className="space-y-4">
                                  <div className="p-3 bg-green-500/10 border border-green-500/20 rounded text-xs text-green-400">
                                    <strong>PRONTO PARA APROVAÇÃO FINAL:</strong> CNH, veículo e
                                    documentos obrigatórios estão regulares.
                                  </div>
                                  <Button
                                    ...
                                    onClick={() => {
                                      ...
                                      const motoristaLista = motoristas?.find(
                                        (m: any) => m.motoristas.id === viewingMotoristaId,
                                      );
                                      if (motoristaLista) {
                                        setActionError(null);
                            ...
                            return (
                              <div className="space-y-4">
                                ...
                                <p className="text-[10px] text-gray-400 text-center italic">
                                  Verifique as pendências e alertas abaixo para habilitar a
                                  aprovação.
                                </p>
                              </div>
                            );
                      ...
                      <Card className="bg-white/5 border-white/10 text-white">
                        <CardContent className="pt-6">
                          ...
                          <div className="mt-1 font-medium flex items-center gap-1">
                            ...
                            {detalhe.motorista.usuarios.cidades?.nome}/
                            {detalhe.motorista.usuarios.cidades?.estado_uf}
                          </div>
                        </CardContent>
                      </Card>
                      <Card className="bg-white/5 border-white/10 text-white">
                        <CardContent className="pt-6">
                          ...
                          <div className="mt-1 font-medium">
                            {new Date(detalhe.motorista.created_at).toLocaleDateString("pt-BR")}
                          </div>
                        </CardContent>
                      </Card>
                      <Card className="bg-white/5 border-white/10 text-white">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-sm">
                      ...
                      <div>
                        ...
                        <div className="font-medium">
                          {detalhe.motorista.usuarios.cpf?.replace(
                            /(\d{3})(\d{3})(\d{3})(\d{2})/,
                            "$1.***.***-$4",
                          ) || "Não informado"}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="font-medium">
                          {detalhe.motorista.usuarios.data_nascimento
                            ? new Date(
                                detalhe.motorista.usuarios.data_nascimento,
                  ...
                  <section className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-sm">
                      <div>
                        ...
                        <div className="font-medium">
                          {detalhe.motorista.cnh_numero || "Não informado"}
                        </div>
                      </div>
                      <div>
                        <div className="text-gray-400">Categoria</div>
                        <div className="font-medium">
                          {detalhe.motorista.cnh_categoria || "Não informado"}
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="font-medium flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            {detalhe.motorista.cnh_validade
                              ? new Date(
                                  detalhe.motorista.cnh_validade + "T12:00:00",
                                ).toLocaleDateString("pt-BR")
                              : "Não informado"}
                            {detalhe.motorista.cnh_validade &&
                              detalhe.motorista.cnh_validade < getHojeBR() && (
                                <Badge variant="destructive" className="h-5 text-[10px] bg-red-600">
                                  VENCIDA
                                </Badge>
                              )}
                          </div>
                            ...
                            detalhe.motorista.cnh_validade < getHojeBR() && (
                              <div className="text-[10px] text-red-500 font-bold uppercase flex items-center gap-1">
                                ...
                                <span className="normal-case font-normal text-gray-400 ml-1">
                                  — Motorista impedido de operar até regularizar a CNH.
                                </span>
                              </div>
                            )}
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-sm">
                      <div>
                        ...
                        <div className="font-medium capitalize">
                          {detalhe.motorista.tipo_chave_pix || "Não informado"}
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="font-medium">
                          {detalhe.motorista.chave_pix
                            ? detalhe.motorista.tipo_chave_pix === "cpf"
                              ? detalhe.motorista.chave_pix.replace(
                                  /(\d{3})(\d{3})(\d{3})(\d{2})/,
                                  "$1.***.***-$4",
                                )
                              : detalhe.motorista.chave_pix.substring(0, 4) +
                                "****" +
                                detalhe.motorista.chave_pix.substring(
                                  detalhe.motorista.chave_pix.length - 2,
                                )
                            : "Não informado"}
                        </div>
                      </div>
                          ...
                          <div className="flex justify-between items-start">
                            <div>
                              <div className="font-bold text-lg">
                                {detalhe.veiculo.marca} {detalhe.veiculo.modelo}
                              </div>
                              <div className="text-sm text-gray-400">
                                {detalhe.veiculo.ano} • {detalhe.veiculo.cor}
                              </div>
                              <div className="mt-2 font-mono text-volt">
                                {detalhe.veiculo.placa}
                              </div>
                            </div>
                            <Badge
                              className={
                                detalhe.veiculo.status_aprovacao === "aprovado"
                                  ? "bg-green-500/20 text-green-500 border-green-500/50"
                                  : detalhe.veiculo.status_aprovacao === "em_analise"
                                    ? "bg-blue-500/20 text-blue-500 border-blue-500/50"
                                    : "bg-amber-500/20 text-amber-500 border-amber-500/50"
                              }
                            >
                  ...
                  {detalhe && (
                    <section className="space-y-3">
                      {(() => {
                        ...
                        const docsEnviados = detalhe.documentos.filter((d: any) =>
                          tiposObrigatorios.includes(d.tipo_documento),
                        );
                        const docsPendentes = docsEnviados.filter(
                          (d: any) => d.status_analise === "pendente",
                        );
                        const docsRecusados = docsEnviados.filter(
                          (d: any) => d.status_analise === "recusado",
                        );
                        const docsCorrecao = docsEnviados.filter(
                          (d: any) => d.status_analise === "correcao_solicitada",
                        );
                        const hoje = getHojeBR();
                        const cnhVencida =
                          detalhe.motorista.cnh_validade && detalhe.motorista.cnh_validade < hoje;
                        ...
                        if (docsEnviados.length < tiposObrigatorios.length)
                          alertas.push(
                            `Faltam ${tiposObrigatorios.length - docsEnviados.length} documentos obrigatórios.`,
                          );
                            ...
                            <CardContent className="p-0">
                                        ...
                                        <div className="flex items-center gap-2">
                                          ...
                                          detalhe.motorista.cnh_validade < getHojeBR() ? (
                                            ...
                                            <Badge
                                              variant="outline"
                                              className={
                                                doc.status_analise === "aprovado"
                                                  ? "text-green-400 border-green-400/30"
                                                  : doc.status_analise === "recusado"
                                                    ? "text-red-400 border-red-400/30"
                                                    : doc.status_analise === "correcao_solicitada"
                              ...
                              {doc?.motivo_recusa && (
                                <div className="px-4 pb-4">
                                  <div
                                    className={`text-[10px] ${doc.status_analise === "correcao_solicitada" ? "text-amber-400 bg-amber-400/5 border-amber-400/10" : "text-red-400 bg-red-400/5 border-red-400/10"} p-2 rounded border`}
                                  >
                                    <strong>
                                      {doc.status_analise === "correcao_solicitada"
                                        ? "CORREÇÃO"
                              ...
                              {reviewingDoc?.id === doc?.id && (
                                <div className="bg-black/20 p-4 border-t border-white/5 space-y-4">
                                  <div className="text-xs font-semibold uppercase text-volt">
                                    Revisar Documento
                                  </div>
                                  <div className="space-y-2">
                  ...
                  <section className="space-y-4">
                    ...
                    <div className="space-y-3">
                      {detalhe.logs && detalhe.logs.length > 0 ? (
                        detalhe.logs.map((log: any) => (
                          <div
                            key={log.id}
                            className="text-xs bg-white/5 p-3 rounded border border-white/5 space-y-1"
                          >
                            <div className="flex justify-between">
                              <span className="font-bold text-volt uppercase">
                                {log.acao.replace("_", " ")}
                              </span>
                            ...
                            {log.estado_anterior && log.estado_novo && (
                              <div className="flex items-center gap-1 text-[10px]">
                                <span className="text-gray-500">
                                  {JSON.stringify(log.estado_anterior)}
                                </span>
