...
import { getCarteiraPassageiro } from "@/lib/carteira-passageiro.functions";
import { resolveDestinationForLoader } from "@/lib/auth-status.functions";

export const Route = createFileRoute("/carteira-passageiro")({
  loader: async () => {
...
function CarteiraPassageiro() {
  const getCarteiraFn = useServerFn(getCarteiraPassageiro);
        ...
        {isLoading || !isHydrated ? (
          ...
          <>
            <div className="bg-zuvvi-volt/5 border border-zuvvi-volt/20 rounded-3xl p-6 space-y-1 text-center">
              ...
              <p className="text-4xl font-black text-zuvvi-volt">
                R$ {formatarMoeda(data.resumo.total)}
              </p>
              <p className="text-[11px] text-muted-foreground">
                {data.resumo.totalCorridas}{" "}
                {data.resumo.totalCorridas === 1 ? "corrida paga" : "corridas pagas"}
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-zuvvi-indigo/40 border border-white/5 rounded-2xl p-4 text-center">
                ...
                <p className="text-base font-black text-white">
                  R$ {formatarMoeda(data.resumo.hoje)}
                </p>
              </div>
              <div className="bg-zuvvi-indigo/40 border border-white/5 rounded-2xl p-4 text-center">
                <p className="text-[9px] text-muted-foreground uppercase tracking-widest mb-1">
                ...
                <p className="text-base font-black text-white">
                  R$ {formatarMoeda(data.resumo.semana)}
                </p>
              </div>
              <div className="bg-zuvvi-indigo/40 border border-white/5 rounded-2xl p-4 text-center">
                <p className="text-[9px] text-muted-foreground uppercase tracking-widest mb-1">
                ...
                <p className="text-base font-black text-white">
                  R$ {formatarMoeda(data.resumo.mes)}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              {data.historico.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center space-y-3 opacity-60">
                  ...
                  <p className="text-sm font-medium">Nenhuma corrida paga ainda.</p>
                </div>
              ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest text-muted-foreground mb-1">
                        ...
                        {item.dataFinalizacao
                          ? format(new Date(item.dataFinalizacao), "dd 'de' MMMM, HH:mm", {
                              locale: ptBR,
                            })
                          : "Data não disponível"}
                      </div>
                      <p className="text-sm font-bold truncate">
                        {item.destinoNome || "Destino não informado"}
                      </p>
