  ...
  if (!userRecord) {
    ...
    if (insertError) {
      ...
      return {
        ...
        error: "Erro na sincronização de perfil. Tente novamente.",
      };
    }
 ...
 */
export const resolveDestinationForLoader = createServerFn({ method: "GET" }).handler(async () => {
...
export const getAuthStatus = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const isRegistrationComplete =
    isAdmin ||
    !!(userRecord?.cpf && userRecord?.celular && userRecord?.data_nascimento && userRecord?.city);
